...
export default function FxTicker() {
  const pairs = TICKER_PAIRS.map(p => ({ ...p, rate: getMarketRate(p.base, p.target) }))
  const items = [...pairs, ...pairs]
  return (
    <div className="bg-indigo-950 text-indigo-200 text-xs py-2 overflow-hidden border-b border-indigo-900 relative">
