      ...
      <div className="max-w-2xl mx-auto text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto">
          <div className="flex-1 relative">
            ...
            <input type="number" value={monthly} onChange={e=>{setMonthly(e.target.value);setResult(null)}} placeholder="50000" min="0" className="w-full bg-white/10 border border-white/20 rounded-xl pl-8 pr-4 py-3.5 text-white placeholder:text-white/40 focus:outline-none focus:border-white/50 font-mono text-lg transition-all"/>
          </div>
          <button onClick={calc} className="bg-white hover:bg-indigo-50 text-indigo-700 font-bold px-6 py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 flex-shrink-0"><TrendingUp className="w-4 h-4"/>Calculate</button>
        </div>
        {result!==null&&(
          <div className="mt-8 bg-white/10 border border-white/20 rounded-2xl p-6 animate-fadeup">
            ...
            <p className="text-indigo-200 text-sm mt-2">Based on avg 4.2% fee overpayment across unoptimized corridors.</p>
