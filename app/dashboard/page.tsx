...
export default function DashboardPage() {
  ...
  const loadSims = async () => {
    ...
    // Inject the current session so RLS works
  ...
  if (loading || fetching) return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        ...
        <p className="text-sm text-[var(--text3)]">Loading your simulations...</p>
      </div>
    </div>
  )
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        {error && (
          <div className="mb-5 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 rounded-2xl p-4 text-rose-600 dark:text-rose-400 text-sm">
            ...
            <div className="mt-1 text-xs opacity-80">Make sure you ran the SQL schema in Supabase and that Row Level Security policies are enabled.</div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
          {[
            ...
            { label: 'Total Saved (USD)', val: '$' + totalSaved.toFixed(0), color: 'text-emerald-600 dark:text-emerald-400' },
            { label: 'Avg Route Score',   val: (avgScore*100).toFixed(0) + '%', color: 'text-violet-600 dark:text-violet-400' },
            { label: 'Corridors Used',    val: corridors.length,           color: 'text-amber-600 dark:text-amber-400' },
          ].map(s => (
