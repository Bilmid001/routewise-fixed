    ...
    <main className="min-h-screen bg-[var(--bg)] grid-bg text-[var(--text)]">
      ...
      <section className="pt-16 sm:pt-24 pb-16 sm:pb-20 px-4 sm:px-6 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center relative">
          ...
          {/* Trust badges */}
          <div className="mt-10 flex flex-wrap justify-center gap-3 sm:gap-4 text-xs text-[var(--text3)]">
            {['🔒 256-bit SSL', '🛡️ SOC 2 Ready', '🚫 No data sold', '⚡ Real-time analysis', '🌍 15 currencies'].map(b => (
              <span key={b} className="bg-[var(--card)] border border-[var(--border)] px-3 py-1.5 rounded-full">{b}</span>
            ))}
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 items-center">
            <div>
              ...
              <h2 className="font-display font-bold text-3xl sm:text-4xl mb-4 leading-tight">
                {t.prob_title.split('hidden')[0]}<span className="text-rose-500">hidden</span>{t.prob_title.split('hidden')[1] || ' payment fees'}
              </h2>
              ...
              <ul className="space-y-2">
                {['4 payment rails with different fee structures', 'FX spreads vary from 0.8% to 3%+', 'Settlement times: 1 hour to 48 hours', '15 currency corridors supported'].map(i => (
                  <li key={i} className="flex items-center gap-3 text-[var(--text2)] text-sm">
                    <CheckCircle2 className="w-4 h-4 text-indigo-500 flex-shrink-0" />{i}
                  </li>
                ))}
              </ul>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {[
                ...
                { l: 'Max Savings',         v: '8%',   c: 'text-emerald-600 dark:text-emerald-400' },
                { l: 'Currencies',          v: '15',   c: 'text-violet-600 dark:text-violet-400' },
              ].map(s => (
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[
              ...
              { n: '04', title: 'Optimal Route',     desc: 'Receive best route with AI explanation and confidence meter.' },
            ].map(s => (
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {[
              ...
              { icon: Zap,       title: 'AI Scoring',        desc: 'Weighted scoring balances settlement time against transaction cost.' },
              { icon: Lock,      title: 'Secure Auth',       desc: 'Sign in with Google or email. Every simulation saved to your account.' },
              { icon: Star,      title: 'Personal History',  desc: 'All your simulations saved forever. Export CSV, track your savings.' },
              { icon: Shield,    title: 'Risk Analysis',     desc: 'Transaction risk assessed per corridor — volatility, settlement, reliability.' },
            ].map(f => (
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6 border-t border-[var(--border)]">
        <div className="max-w-2xl mx-auto text-center">
          <div className="text-indigo-600 dark:text-indigo-400 text-xs font-bold px-4 py-1.5 rounded-full inline-block mb-6 uppercase tracking-widest border border-indigo-200 dark:border-indigo-700/40">{t.cta_badge}</div>
      ...
      <footer className="border-t border-[var(--border)] py-6 sm:py-8 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left">
          ...
          <p className="text-[var(--text3)] text-xs">{t.footer_disc}</p>
        </div>
      </footer>
      <AIChatWidget />
    </main>
