      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        <div className="mb-6 sm:mb-8 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-[var(--text2)] mt-2 text-sm sm:text-base">Compare all 4 payment rails across 15 currencies — instant results.</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            {!user&&<Link href="/auth/login" className="flex items-center gap-1.5 text-sm text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-700 px-3 py-2 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all"><LogIn className="w-3.5 h-3.5"/>Sign in to save history</Link>}
            {saveStatus==='saved'&&<span className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 px-3 py-1.5 rounded-full">✓ Saved to history</span>}
            {saveStatus==='saving'&&<span className="text-xs text-indigo-500 font-semibold">Saving...</span>}
            {saveStatus==='error'&&<span className="text-xs text-rose-500 font-semibold bg-rose-50 dark:bg-rose-500/10 border border-rose-200 px-3 py-1.5 rounded-full">⚠ Save failed</span>}
            <button onClick={()=>setShowHistory(!showHistory)} className="flex items-center gap-1.5 text-[var(--text2)] text-sm border border-[var(--border)] px-3 py-2 rounded-lg hover:border-indigo-300 transition-all">
              <History className="w-3.5 h-3.5"/>History{history.length>0&&<span className="bg-indigo-600 text-white text-xs px-1.5 rounded-full">{history.length}</span>}
            </button>
            {result&&<button onClick={exportCSV} className="flex items-center gap-1.5 border border-[var(--border)] text-[var(--text2)] text-sm px-3 py-2 rounded-lg hover:border-indigo-300 transition-all"><Download className="w-3.5 h-3.5"/>CSV</button>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-6 sm:gap-8">
          <div className="xl:col-span-2">
            <div className="sticky top-20 bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5 sm:p-6 shadow-sm">
              ...
              {result&&<div className="mt-5 pt-5 border-t border-[var(--border)]"><div className="text-xs text-[var(--text3)] uppercase tracking-widest mb-2">Mid-Market Rate</div><div className="font-mono font-bold text-base sm:text-lg">1 {result.sourceCurrency} = {result.data.marketRate>=1?result.data.marketRate.toFixed(4):result.data.marketRate.toFixed(8)} {result.destinationCurrency}</div></div>}
            </div>
          </div>
          <div className="xl:col-span-3 space-y-5 sm:space-y-6">
            {!result&&!isLoading&&(
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-12 sm:p-16 text-center shadow-sm">
                ...
                <p className="text-[var(--text2)] text-sm sm:text-base">Enter your transaction details and click Simulate Payment.</p>
              </div>
            )}
            {result&&<>
              ...
              <AIInsightPanel bestRoute={result.data.bestRoute} sourceCurrency={result.sourceCurrency} destinationCurrency={result.destinationCurrency} amount={result.amount}/>
            </>}
          </div>
        </div>
      </div>
