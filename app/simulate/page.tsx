...
export default function SimulatePage() {
  ...
  const handleExportCSV = () => {
    ...
    const rows = result.data.routes.map((r: any) => [
      r.name, r.fxAdjustedRate.toFixed(4), r.convertedAmount.toFixed(2),
      r.totalFee.toFixed(2), r.finalReceived.toFixed(2),
      r.settlementHours, (r.reliabilityScore * 100).toFixed(0) + '%', r.score.toFixed(3),
    ])
    const csv = [hdrs, ...rows].map(r => r.join(',')).join('\n')
    const a = document.createElement('a')
    a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }))
    a.download = 'routewise-' + Date.now() + '.csv'
    a.click()
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        <div className="mb-6 sm:mb-8 flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {saveStatus === 'error'  && <span className="text-xs text-rose-500 font-semibold bg-rose-50 dark:bg-rose-500/10 border border-rose-200 px-3 py-1.5 rounded-full">⚠ Save failed — check Supabase RLS</span>}
            ...
            {result && (
              <button onClick={handleExportCSV} className="flex items-center gap-1.5 border border-[var(--border)] text-[var(--text2)] text-sm px-3 py-2 rounded-lg hover:border-indigo-300 transition-all">
            ...
            <div className="space-y-2">
              {history.map(h => (
                <button key={h.id} onClick={() => setResult(h.result)}
                  className="w-full flex flex-wrap sm:flex-nowrap items-center justify-between bg-[var(--bg2)] border border-[var(--border)] hover:border-indigo-300 rounded-xl px-4 py-3 transition-all text-left gap-2">
                  <div className="flex items-center gap-3">
                    <span className="font-semibold text-sm">{h.src}→{h.dst}</span>
              ...
              {result && (
                <div className="mt-5 pt-5 border-t border-[var(--border)]">
                  ...
                  <div className="font-mono font-bold text-base sm:text-lg">
                    1 {result.sourceCurrency} = {result.data.marketRate >= 1 ? result.data.marketRate.toFixed(4) : result.data.marketRate.toFixed(8)} {result.destinationCurrency}
                  </div>
                </div>
              )}
            </div>
