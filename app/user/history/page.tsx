    ...
    <main className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        <div className="flex gap-1 bg-[var(--bg2)] border border-[var(--border)] rounded-xl p-1 mb-6 w-fit">
          {(['history','alerts'] as const).map(t=><button key={t} onClick={()=>setTab(t)} className={"px-4 py-2 rounded-lg text-sm font-semibold capitalize transition-all "+(tab===t?'bg-[var(--card)] shadow text-[var(--text)] border border-[var(--border)]':'text-[var(--text2)] hover:text-[var(--text)]')}>{t==='history'?'Simulations':'Rate Alerts'}</button>)}
        </div>
        {tab==='history'&&<div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-sm overflow-hidden">
        ...
        {tab==='alerts'&&<div className="space-y-4">
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5 sm:p-6 shadow-sm">
            <h2 className="font-bold text-base mb-2 flex items-center gap-2"><Bell className="w-4 h-4 text-indigo-500"/>Create Rate Alert</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
              ...
              <select value={alertForm.currency_pair} onChange={e=>setAlertForm(p=>({...p,currency_pair:e.target.value}))} className="w-full bg-[var(--bg2)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] focus:outline-none focus:border-indigo-500 transition-all appearance-none text-sm">
                ...
                {CURRENCIES.slice(0,8).flatMap(a=>CURRENCIES.filter(b=>b.code!==a.code).slice(0,4).map(b=><option key={a.code+b.code} value={a.code+'/'+b.code}>{a.flag}{a.code}/{b.flag}{b.code}</option>))}
              </select></div>
              <div><label className="block text-xs font-bold text-[var(--text3)] uppercase tracking-widest mb-2">Threshold</label><input type="number" value={alertForm.threshold} onChange={e=>setAlertForm(p=>({...p,threshold:e.target.value}))} placeholder="e.g. 1580" className="w-full bg-[var(--bg2)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] placeholder:text-[var(--text3)] focus:outline-none focus:border-indigo-500 transition-all font-mono"/></div>
              <div><label className="block text-xs font-bold text-[var(--text3)] uppercase tracking-widest mb-2">Direction</label><select value={alertForm.direction} onChange={e=>setAlertForm(p=>({...p,direction:e.target.value}))} className="w-full bg-[var(--bg2)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] focus:outline-none focus:border-indigo-500 transition-all appearance-none text-sm"><option value="below">Rate drops below</option><option value="above">Rate rises above</option></select></div>
            </div>
          ...
          {alerts.length===0?<div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-10 text-center shadow-sm"><Bell className="w-10 h-10 text-[var(--text3)] mx-auto mb-3"/><p className="text-[var(--text2)] font-semibold">No rate alerts yet</p></div>
          :alerts.map(a=>(
            <div key={a.id} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4 sm:p-5 shadow-sm flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className={"w-9 h-9 rounded-xl flex items-center justify-center "+(a.is_active?'bg-emerald-100 dark:bg-emerald-500/20':'bg-slate-100 dark:bg-slate-700')}>{a.is_active?<Bell className="w-4 h-4 text-emerald-600 dark:text-emerald-400"/>:<BellOff className="w-4 h-4 text-slate-400"/>}</div>
                <div><div className="font-bold text-sm">{a.currency_pair}</div><div className="text-xs text-[var(--text3)]">Alert when rate {a.direction} <span className="font-mono font-bold">{a.threshold}</span></div></div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={()=>delAlert(a.id)} className="text-rose-500 hover:text-rose-400 p-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors"><Trash2 className="w-3.5 h-3.5"/></button>
              </div>
            </div>
          ))}
        </div>}
