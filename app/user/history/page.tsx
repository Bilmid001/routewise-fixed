...
import { Download, Bell, BellOff, Plus, Trash2, BarChart3 } from 'lucide-react'
...
export default function HistoryPage() {
  ...
  const loadData = async () => {
    ...
    const [simsRes, alertsRes] = await Promise.all([
      supabase.from('simulations').select('*').eq('user_id', user!.id).order('created_at', { ascending: false }).limit(100),
      supabase.from('email_alerts').select('*').eq('user_id', user!.id).order('created_at', { ascending: false }),
    ])
    if (simsRes.error)   setError(simsRes.error.message)
  ...
  const addAlert = async () => {
    ...
    const supabase = await getClient()
    const { data } = await supabase.from('email_alerts').insert({
      user_id: user!.id, currency_pair: alertForm.currency_pair,
      threshold: parseFloat(alertForm.threshold), direction: alertForm.direction, is_active: true,
    }).select().single()
  ...
  const exportCSV = () => {
    ...
    const rows = sims.map(s => [s.amount, s.source_currency, s.destination_currency, s.best_route_name, s.savings_amount, new Date(s.created_at).toLocaleDateString()])
    const csv = [hdrs, ...rows].map(r => r.join(',')).join('\n')
    const a = document.createElement('a')
    a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }))
    a.download = 'my-simulations.csv'; a.click()
  }
    ...
    <main className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        <div className="mb-6 sm:mb-8">
          ...
          <p className="text-[var(--text2)] mt-2 text-sm">All your saved simulations and rate alert settings</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
          {[
            { label: 'Total Simulations', val: sims.length },
            { label: 'Total Saved (USD)',  val: '$' + totalSaved.toFixed(0) },
            { label: 'Corridors Used',     val: corridors.length },
            { label: 'Active Alerts',      val: alerts.filter(a => a.is_active).length },
          ].map(s => (
        ...
        <div className="flex gap-1 bg-[var(--bg2)] border border-[var(--border)] rounded-xl p-1 mb-6 w-fit">
          {(['history','alerts'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={"px-4 py-2 rounded-lg text-sm font-semibold capitalize transition-all " + (tab === t ? 'bg-[var(--card)] shadow text-[var(--text)] border border-[var(--border)]' : 'text-[var(--text2)] hover:text-[var(--text)]')}>
        ...
        {tab === 'history' && (
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-sm overflow-hidden">
            <div className="p-5 border-b border-[var(--border)] flex items-center justify-between">
              ...
              {sims.length > 0 && (
                <button onClick={exportCSV} className="flex items-center gap-1.5 text-sm border border-[var(--border)] text-[var(--text2)] px-3 py-2 rounded-lg hover:border-indigo-300 transition-all">
            ...
            {sims.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="font-semibold text-[var(--text2)]">No simulations saved yet</p>
                <p className="text-[var(--text3)] text-sm mt-1">Run a simulation and it will appear here automatically.</p>
              </div>
            ) : (
        ...
        {tab === 'alerts' && (
          <div className="space-y-4">
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5 sm:p-6 shadow-sm">
              <h2 className="font-bold text-base sm:text-lg mb-2 flex items-center gap-2"><Bell className="w-4 h-4 text-indigo-500" />Create Rate Alert</h2>
              <p className="text-sm text-[var(--text3)] mb-4">Get emailed when a currency pair crosses your threshold.</p>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                <div>
                  <label className="block text-xs font-bold text-[var(--text3)] uppercase tracking-widest mb-2">Currency Pair</label>
                  <select value={alertForm.currency_pair} onChange={e => setAlertForm(p => ({ ...p, currency_pair: e.target.value }))}
                    className="w-full bg-[var(--bg2)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] focus:outline-none focus:border-indigo-500 transition-all appearance-none text-sm">
                    <option value="">Select pair...</option>
                    {CURRENCIES.slice(0,8).flatMap(a => CURRENCIES.filter(b => b.code !== a.code).slice(0,4).map(b => (
                      <option key={a.code+b.code} value={a.code+'/'+b.code}>{a.flag}{a.code}/{b.flag}{b.code}</option>
                    )))}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-[var(--text3)] uppercase tracking-widest mb-2">Threshold Rate</label>
                  <input type="number" value={alertForm.threshold} onChange={e => setAlertForm(p => ({ ...p, threshold: e.target.value }))} placeholder="e.g. 1580"
                    className="w-full bg-[var(--bg2)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] placeholder:text-[var(--text3)] focus:outline-none focus:border-indigo-500 transition-all font-mono" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-[var(--text3)] uppercase tracking-widest mb-2">Alert When</label>
                  <select value={alertForm.direction} onChange={e => setAlertForm(p => ({ ...p, direction: e.target.value }))}
                    className="w-full bg-[var(--bg2)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] focus:outline-none focus:border-indigo-500 transition-all appearance-none text-sm">
              ...
              <button onClick={addAlert} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-5 py-2.5 rounded-xl transition-all text-sm">
            ...
            {alerts.length === 0 ? (
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-10 text-center shadow-sm">
                ...
                <p className="text-[var(--text3)] text-sm mt-1">Create an alert above to get notified when rates move.</p>
              </div>
            ) : alerts.map(a => (
              <div key={a.id} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4 sm:p-5 shadow-sm flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <span className={"text-xs font-bold px-2.5 py-1 rounded-full " + (a.is_active ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-700 text-slate-500')}>
                    {a.is_active ? 'Active' : 'Inactive'}
                  </span>
                  <button onClick={() => deleteAlert(a.id)} className="text-rose-500 hover:text-rose-400 p-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors">
