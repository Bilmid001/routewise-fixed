...
export const FX_RATES: Record<string,number> = {
  ...
  'NGN-GHS':0.01,'GHS-NGN':100,'NGN-KES':0.082,'KES-NGN':12.2,
  'GBP-EUR':1.170,'EUR-GBP':0.855,'CAD-NGN':1155,'NGN-CAD':0.000866,
  'AUD-NGN':1039,'NGN-AUD':0.000963,'AED-INR':22.7,'INR-AED':0.044,
}
export function getMarketRate(base:string,target:string):number {
  if(base===target) return 1
  const d = FX_RATES[base+'-'+target]; if(d) return d
  const u = FX_RATES[base+'-USD']; const f = FX_RATES['USD-'+target]
...
export const TICKER_PAIRS = [
  ...
  {pair:'USD/CNY',base:'USD',target:'CNY'},{pair:'USD/BRL',base:'USD',target:'BRL'},
]
